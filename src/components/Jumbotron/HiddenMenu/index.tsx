...
const HiddenMenu: FC<IHiddenMenuProps> = (props: IHiddenMenuProps) => {
  const { isVisible, toggleMenu } = props;

  return (
    <div
      className={`hidden-menu-container w-screen h-screen flex fixed top-0 z-50 transition-transform duration-500 ease-in-out ${
    ...
    >
      ...
      <div className="hidden-menu-right w-1/2 h-svh bg-black pl-[97px] ">
        <div className="hidden-menu-navigation py-[121px] relative">
          ...
          <div
            className="close-btn absolute right-14 top-14 "
            onClick={toggleMenu}
          >
            <Image className="w-20 h-20" src={CloseBtn} alt="close-button" />
          </div>
        </div>
